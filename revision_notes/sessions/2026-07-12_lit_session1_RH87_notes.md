...
- **2026-07-23 — CONTENT pass, resuming Phase 1 at §3 (global precipitation patterns).**
...
· 6 = fault map. *This session is in Phase 1 (reading not finished).*
**Batches** = how I grouped my questions to work them, not a workflow stage:
1 = method spine (done) · 2 = Table 2 + article claims (done) · 3 = method's limits
(pending; also incomplete — the unread half's questions aren't in it yet).
**V.N** = a verification item (Phase 3.5). **F.N** = a fault-map lesson (Phase 6).

**Phase 1: INCOMPLETE. Phase 2–3: Batches 1–2 COMPLETE. Batch 3 pending.**
Next action: RESUME READING at §3 (global precipitation patterns). Then extend the [C]/[G]/[S]
inventory with questions from the rest of the paper before touching Batch 3 or the exam.
...
> **⚠️ REFINED 22 Jul (F.14): a figure is immune to TYPESETTING error, not to READING error.** Bars near the 50th percentile are near-zero length and unresolvable at scan resolution. **A figure is a strong witness for SENSE and SEASON, a weak one for TOTALS.** Where table and prose agree verbatim, a discordant bar count is evidence about my counting, not about the paper.
...
| **ING 20/25** | ✅ | ❌ **§4a states NO count** | ⚠️ **VOID — the 16 Jul count (11/12) failed its own pre-registration; panel identity unconfirmed (V.12)** | ⚠️ **NOT double-witnessed — Table 2 + §4a's ≥80% constraint only** |
| **Gulf/N. Mex 18/22** | ✅ | ❌ **no RH87 prose — imported from RH86** | ❌ not in RH87 | ⚠ **→ verify in RH86 itself** |


---

## ⚠️ HOW TO WEIGHT CLAUDE
> *Calibration gathered the hard way. Read before accepting any `[CLAUDE]` tag.*

**Claude's claims about WHAT THE SOURCE SAYS: 0 for 4** — the "fabricated premise" (F.2), the western-US scope (F.7), the SWA-aridity prediction (F.8), the OCR hypothesis (F.12). Add to that the **V.12 panel identification**, which sent a count to the wrong figure.
**Claude's DERIVATIONS from stated premises have held up every time I checked them** — the coherence formula's behaviour · the gamma-pooling arithmetic · the binomial figures · the two-way-split mechanics · the four-failure-mode structure · the three-witness triangulation.
...
> *(General form of this rule, plus the other transferable faults from this session, now lives in `revision_notes/lessons.md`.)*
...
- **[C10] [MINE]** §1: the introduction's arc, and — are the target regions pre-judged or
  discovered? — **ANSWERED 22 Jul · one inversion caught. Extends [C3].**
...
- **[G8] [MINE]** §2: what does the modern equivalent of RH87's data section look like?
  Station networks added/discontinued since 1983; gridded products (GPCC, CRU TS, GHCN);
  the satellite era (GPCP/CMAP, TRMM→GPM); reanalysis. **What is the honest modern
  substitute for "1700 stations, 1875–1983"?** — **PENDING (logged 22 Jul, unanswered).**
  - **Sub-lookup:** how many ENSO episodes since 1983? **⚠ Definition-dependent** — RH87's
    episode list predates the ONI, so an ONI-based count is NOT comparable to the paper's
    25 without stating the definition. *Primary source: CPC historical ONI episode table.*
- **[G3] EXTENDED 22 Jul** — re-derived independently on the §2 re-read. The modern-replication
  question now has three parts: (1) modern data landscape [G8]; (2) re-run the method as
  specified; (3) do the regions shift, and is the shift scientifically meaningful (vs.
  network/product artefact)? *Confirms the Block B default: SEPARATE deliverable, not NB4.*
  *(See also [P2].)*
...
- **[S6]** **Table 2's reliability, given the SSA anomaly.** — **PARTIALLY RESOLVED (V.11): the anomaly is isolated; NAU is double-witnessed. ⚠️ ING's second witness is VOID — V.12 reopened 22 Jul.**
...
### ✅ RESOLVED — collapsed to one line each (full working is in git history, commits b86f6ee / 1911b8f)
- **V.1** — the 0.80 coherence criterion exists, is inherited from RH86, and is **NOT enforced** (CEN sits at 0.77).
- **V.2** — both southern-US rows are present; Fig. 21 shows dashed (wet) Oct(0)–Mar(+). [C4]'s alarm was a miss, not a hole.
- **V.3** — `wet + dry = total` in all 19 rows → binary classification, no neutral category.
- **V.4** — **two** transforms, not one: ranks (global screen) → gamma percentiles (regional analysis).
- **V.5** — the missing Australian region is **SWA**, excluded at **season identification** (§4b). → NB4 failure mode #3.
- **V.6** — RH86 attribution stated in three places (Table 2 asterisk · §5 · §4g). **Footnote rule: the southern-US flood case cites RH 1986, MWR 114.**
- **V.7** — Claude's western-US claim was mis-scoped. §5's two limitations: sign-flipping regions invisible by construction; station coverage gaps (west coast of S. America, eastern Pacific).
- **V.8** — the gamma fit is **per calendar month**, resolved by internal consistency (a pooled fit would maximally encode the annual cycle, making the paper's own sentence false).
- **V.11** — Table 2 is reliable where the article touches it, with one gap: **NAU double-witnessed** (§4b verbatim "22 out of 26"); **ING has no prose count**; Gulf/N. Mexico has no RH87 prose at all.
- **V.13** — the SSA row is a **confirmed discrepancy**: Table 2 (19/18/1) vs prose + Fig. 17b (both 16). NSA is a clean control, so the error is isolated to SSA. Cause undetermined. **SSA counts are not relied on**; sense + season are consistent across all three witnesses.
- **V.15** — §4b's verbatim count is **"22 out of 26"**, not 25; the V.11 entry had mis-transcribed it. NAU is 22/26 and was never in doubt.
...
- **V.12 — ⚠️ REOPENED 22 Jul. THE 16 JUL CLOSURE IS VOID.** Recorded result was "Fig. 4c:
  11 of 12 marked episodes dry." **ING's row is 20 of 25 (80.0%); the count gives 91.7% on a
  denominator of 12.** V.12's own spec pre-registered *"expect 25 marked episodes, of which
  20 fall below the median"* — **the count failed both numbers and was closed regardless.**
  - **⚠ HYPOTHESIS (Claude's, 0-for-4 — TEST IT): the panel counted was not ING's.** Pacific
    panels have unequal record lengths (CP = 8, ING = 25); a ~12-episode panel fits some other
    region. The "ING is panel (c)" identification originated with Claude, not the page.
  - **ACTION at §4a: read the panel LABELS in Fig. 4 (Fig. 7a prints its region name on the
    panel), identify ING's panel by name, then straightedge-count. Expect 25 marked, 20 below.**
  - **Until then: ING rests on Table 2's row + §4a's ≥80% constraint. NOT double-witnessed.**
  **STATUS: OPEN.**
...
- **V.14 — §2 says the longest series span 25 ENSO episodes; NAU's row is 26.** ING is 25
  (consistent). **What makes NAU's record one episode longer?** Candidate sitting in §2: the
  Australian data were supplemented by several dozen long-period stations from the Australian
  National Climate Center. **⚠ CLAUDE'S HYPOTHESIS — 0-for-4, test it, do not assume it.**
  **STATUS: OPEN — resolve at §4b.**

- **V.15 — ✅ RESOLVED 22 Jul (opened and closed same sitting). §4b's verbatim count is "22 out of 26", NOT "22 out of 25".** The V.11 entry mis-transcribed it and asserted "matches Table 2 exactly" while the two disagreed. **Re-read off the page 22 Jul: §4b = 26, Table 2 = 26, Fig. 7a = NAU's panel (region name printed on it). NAU is 22/26, double-witnessed, and was never in doubt.** *The Fig. 7a bar count that appeared to give 25 was a reading error — bars near the median are unresolvable (F.14) — and was seeded by this very typo (F.16).* **No change to NAU's numbers anywhere; the transcription was the only thing wrong.**
...
### Batch 4 — resumed read from the top (22 Jul onward)
*Phase 1 restarted: the original stopping point was lost. Questions from this pass continue
the existing numbering (C10+, G8+, S7+).*

**[C10] — §1's arc, and the pre-judged/discovered inversion.** `[MY READING, checked vs. pp. 1606–07]`
- **Arc, confirmed:** Walker's global ENSO documentation was *motivated by* Indian monsoon
  prediction, then expanded to global precipitation → a catalogue of prior results (Berlage
  1966 · Stoeckenius 1981 · Rasmusson & Wallace 1983 · Rasmusson & Carpenter 1983 ·
  Shukla & Paolino 1983 · Horel & Wallace 1981 · **Hastenrath 1978** · Kousky et al. 1984 ·
  Douglas & Englehart 1981 · **Ropelewski & Halpert 1986**) → this paper's purpose.
  *(⚠ I first wrote Hastenrath 1987; the page reads 1978.)*
- **Purpose, on three axes** *(my reframe — keep):* **spatial** = geographical extent ·
  **scalar** = magnitude · **temporal** = phase and duration.
- **⚠ INVERSION CAUGHT.** I read the aim as regions "already judged" to have strong signals.
  **The page says *identifying* such regions — identification is the OUTPUT of the analysis,
  not a precondition.** If the regions were pre-selected, RH87 would carry the very flaw it
  criticises. *(= [C3]: prior work's shared weakness is that the investigator chose regions
  and timing in advance; §5 — the core regions and seasons are themselves determined by the
  analysis.)*
  → **ARTICLE RULE: never describe RH87's regions as "known" regions. They are DERIVED.**
- **§1 roadmap:** §2 = data · §3 = global patterns · §4 = regional analyses. *(§5 already mined.)*
...
### Questions — capstone set (drafted 22 Jul 2026; exam remains DEFERRED until NB4 runs end-to-end)

- **Q1 (method — the pipeline).** Walk the RH87 method in order, from raw station
  monthly precipitation to a finished Table 2 row. Name each stage and what it
  produces. (Target: rank → percentile → 24-month composite → first-harmonic fit →
  vector map / coherence screen → regional index → "season" extraction from phase →
  per-episode consistency count.)
- **Q2 (notation).** What do the (0) and (+) year labels mean? For the 2026–27
  episode, which calendar months does ING's dry "season" Jun(0)–Nov(0) span?
- **Q3 (definition — re-test of logged fault).** Define coherence *exactly as
  computed* (p. 1608). What do high vs. low values indicate about a region's
  harmonic vectors?
- **Q4 (trap — NB4's thesis).** Why do the authors say harmonic vectors alone are
  "not sufficient," and what replaces them? Describe the monster-episode pathology:
  what does the amplitude/composite screen show, what does the per-episode count
  show, and why does the count catch what the screen misses? Then the planted
  Australia question: the abstract counts four Australian regions but Fig. 5
  outlines five — which one failed, and at which stage (coherence screen vs.
  consistency count)?
- **Q5 (payload — re-test of logged fault).** Name Table 2's exact columns. Why
  must the wet/dry *sense* be derived from the counts rather than hardcoded (in
  NB4 and in any citation of a row)?
- **Q6 (provenance trap).** The Gulf of Mexico–northern Mexico wet signal
  (Oct(0)–Mar(+), 18/22) appears in Table 2. Which paper must a footnote citing it
  point to, and why is the error so easy to make?
- **Q7 (rows the article touches).** For ING and NAU: give season, sense, and
  episode counts, and state which specific article/tracker claim each supports.
- **Q8 (internal inconsistency — V.13).** For the SSA row: which three places in
  the paper disagree, about what quantity, and what remains consistent across all
  three?
- **Q9 (figure convention — NB4 spec input).** Describe Fig. 5's vector-clock
  convention: the 24-month clock orientation, what the amplitude rings mean, what
  solid vs. dashed region outlines encode, and what a single vector's phase and
  length each represent.
- **Q10 (synthesis — the epistemology bridge).** RH87 uses a two-stage structure:
  screening diagnostic first, per-event verification second. Map both stages onto
  NB3/the article: what plays the role of the empirical base rate, what plays the
  per-episode probability (IRI ELR terciles), and why must these two evidence
  types stay distinct in Section 2's prose?
...
2. Route ripe items **and delete them from here** — a routed item that stays is tomorrow's duplicate.
3. `git add` → `git commit -m "lit s1 (RH87): <one-line actual state>"` → `git push`.
4. Reconcile: `git status` clean; **read the diffstat**; tracker and ledger match reality.

## What this file is — and is not
**IS:** the reading and comprehension record for RH87 — questions, answers, the paper's method, the Table 2 audit, open verifications, the exam.
**IS NOT:** a lessons file (→ `lessons.md`), a build spec (→ `sessions/2026-07-22_nb4_build_notes.md`), an article ledger (→ `article_ledger.md`), or a task queue (→ `TODO.md` / tracker).
*Reorganised 22 Jul 2026: this file had absorbed all four and had grown to 562 lines.*
